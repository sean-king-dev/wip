...
<head>
    <meta charset="UTF-8">
    <title>MODX Resource Layout Finder</title>
    <style>
        html, body {
...
<body>
    <main>
        <h2>Resource/ Content Block Layouts</h2>
    </main>
    <form method="get">
...
if (isset($_GET['id']) && is_numeric($_GET['id'])) {
    ...
    if ($result && $result->num_rows > 0) {
        ...
        $fieldResult = $conn->query('SELECT id, name FROM modx_contentblocks_layout');

        if ($fieldResult && $fieldResult->num_rows > 0) {
            echo "<h2>Layouts found in resource $resourceId:</h2><div class='grid-container'>";
            $foundAny = false;
            ...
            if (!$foundAny) {
                echo "(No content block layouts found)\n";
            }
            echo '</div>';
        } else {
            echo '<p>No fields found in modx_contentblocks_layout.</p>';
        }
    } else {
